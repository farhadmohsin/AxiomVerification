...
def Gaussian_voters_candidates(n, m, d=2):
    ...
    g = np.random.multivariate_normal(center, np.eye(d)*0.2, size = n)
...
def gaussian_profile(n, m, d = 2):
    g, candidates = Gaussian_voters_candidates(n, m, d)
    votes = random_pref_profile(g, candidates)
    return votes

def uniform_euclid_profile(n, m, d = 2):
    g, candidates = uniform_voters_candidates(n, m, d)
    votes = random_pref_profile(g, candidates)
    return votes
