...
class Mechanism():
    ...
    """
    def lexWinner(self, profile):
        '''computing a single winner using lexicographic tiebreaking'''
    ...
    def satCondorcet(self, profile):
        winner = self.lexWinner(profile)
        wmg = profile.getWmg()
        CW = []
        for cand1 in profile.candMap:
            IsCW = 1
            for cand2 in profile.candMap:
                if cand2 != cand1 and wmg[cand1][cand2] <= 0:
                    IsCW = 0
            if IsCW == 1:
                CW.append(cand1)
        # print ( "CW=:" + str(CW) + ", winner =" + str(winner))
        if len(CW) > 0 and CW[0] != winner:
            return 0
        else:
            return 1

    def satParticipation(self, profile):
        #print("I am here")
        index = 0
        winner = self.lexWinner(profile)
        for vote in profile.preferences:
            index += 1
            #print("computing "+str(index) + " out of "+ str(len(profile.preferences)))
            vote.count -= 1
            new_winner = self.lexWinner(profile)
            vote.count += 1
            if new_winner != winner and vote.wmgMap[new_winner][winner] > 0:
                return 0
        return 1

class MechanismEO(Mechanism):


    def quickMOVCheck(self, profile, old_wmg, edgelist):
        '''
        input: preference profile in class Profile; old_wmg: the WMG of the profile; edgelist is the list of all edges
        return: 1 if the MOV is strictly larger than 1; 0 if the MOV is exactly 1'''
        valuelist = []
        for (cand1, cand2) in edgelist:
            valuelist.append(old_wmg[cand1][cand2])
        slist = sorted(valuelist)

        diff = 1
        for i in range(len(slist) - 1):
            if abs(slist[i + 1] - slist[i]) < 5:
                diff = 0
                break
        if diff == 1:
            # print("MOV > 1")
            return 1  # changing one vote does not change the order


    def satParticipation(self, profile):

        winner = self.lexWinner(profile)
        old_wmg = profile.getWmg()
        edgelist = []
        for cand1 in old_wmg.keys():
            for cand2 in old_wmg.keys():
                if cand1 != cand2:
                    edgelist.append((cand1, cand2))

        if self.quickMOVCheck(profile, old_wmg, edgelist):
            # if MOV is > 1, then return true
            return 1

        # otherwise, proceeds with full check
        vote_index = 0
        for vote in profile.preferences:
            vote_index += 1
            #print("computing "+ str(vote_index) + "out of " + str(len(profile.preferences)))
            vote.count -= 1
            # compute new WMG by taking out one copy of a type of vote
            new_wmg = profile.getWmg()
            vote.count += 1
            for pair1, pair2 in itertools.combinations(edgelist, 2):
                # if edge order changes
                if (old_wmg[pair1[0]][pair1[1]] - old_wmg[pair2[0]][pair2[1]])*(new_wmg[pair1[0]][pair1[1]] - new_wmg[pair2[0]][pair2[1]]) <= 0:
                    vote.count -= 1
                    # compute the new winner
                    new_winner = self.lexWinner(profile)
                    vote.count += 1
                    # if a violation of Participation is found, return 0
                    if new_winner != winner and vote.wmgMap[new_winner][winner] > 0:
                        return 0
                    #otherwise check the next type of votes
                    else:
                        break
        #print("EO same")
        return 1
...
class MechanismCopeland(MechanismEO):
    """
    ...
    def quickMOVCheck(self, profile, old_wmg, edgelist):
        scoremap = self.getCandScoresMap(profile)
        scores = sorted(scoremap.values(), reverse=True)
        # if all CP scores are the same, or the highest score is no more than the second highest plus 2, then check min edge
        if len(scores) == 1 or abs(scores[0] - scores[1]) <= 2:
            edges = sorted([abs(old_wmg[c1][c2]) for (c1,c2) in edgelist])
            if edges[0] <= 1:
                return 0
        # otherwise MOV must be strictly larger than 1
        return 1
...
class MechanismMaximin(MechanismEO):
    """
    ...
    def quickMOVCheck(self, profile, old_wmg, edgelist):
        ...
        scoremap = self.getCandScoresMap(profile)
        scores = sorted(scoremap.values(), reverse = True)
        # if all MM scores are the same, or the highest score is no more than the second highest plus 2, then return 0
        if len(scores) == 1 or abs(scores[0]-scores[1]) <= 2:
            return 0
        # otherwise MOV must be strictly larger than 1
        else:
...
class MechanismSchulze(MechanismEO):
    """
...
class MechanismSTV(Mechanism):
    ...
    def lexWinner(self, profile, po=None):
        """
        Returns an integer that represents the winner of a profile under IRV rule under
        fixed tiebreaking order po. By default, it is alphabetical tiebreaking.
        :ivar Profile profile: A Profile object that represents an election profile.
        :ivar List po: A fixed tiebreaking order.
        :return: The single winner
        """
        ordering = profile.getOrderVectors()
        prefcounts = profile.getPreferenceCounts()
        startstate = list(profile.candMap.keys())
        ...
        while len(state):
            ...
            plural_score = self.get_plurality_scores(prefcounts, ordering, state)

            minscore = min(plural_score.values())
            res = [k for k, v in plural_score.items() if v == minscore]
            state.remove(max(res))
        return w

    def getWinners(self, profile):
        """
        Return the PUT-STV winners
        :param profile: profile: A Profile object that represents an election profile.
        :return: list of PUT-winners
        """
        elecType = profile.getElecType()
    ...
    def get_plurality_scores(self, prefcounts, ordering, state):
        plural_score = {}
...
class MechanismBaldwin(Mechanism):
    """
...
class MechanismCoombs(Mechanism):
    def getWinners(self, profile):
        """
...
class MechanismRankedPairs(MechanismEO):
    """
    ...
    def lexWinner(self, profile,):
        '''use alphabetical order to output a single winner'''
        wmg = profile.getWmg()
        m = len(profile.candMap)
        oedges = []
        upset = dict() #for each candidate, maintain a set of alternatives that are more preferred to it
        downset = dict()
        for i in profile.candMap.keys():
            upset[i] = set()
            downset[i] = set()
            for j in profile.candMap.keys():
                if i!= j and wmg[i][j] >= 0:
                    oedges.append((-wmg[i][j] * m * m + i*m + j, (i,j)))
        oedges.sort(reverse=True)
        while len(oedges) >0:
            (w, (i,j)) = oedges.pop()
            ind = dict() #indicator: candidate has an incoming edge

            if j not in downset[i] and j not in upset[i]:
                up = upset[i].union({i})
                down = downset[j].union({j})
                for u in up:
                    downset[u].update(down)
                for d in down:
                    upset[d].update(up)
                possible = []
                for i in profile.candMap.keys():
    ...
    def simple_ranked_pairs(self, profile, tie_breaking_order=None):
        """
        Returns a winner of profile under ranked pairs rule。
        N.B.: tie-breaking: the default tie-breaking is lexical order, e.g.
        [(1, 5), (2, 5), (3, 5), (4, 5), (1, 4), (2, 4), (3, 4), (5, 4), (1, 3),
        (2, 3), (4, 3), (5, 3), (1, 2), (3, 2), (4, 2), (5, 2), (2, 1), (3, 1), (4, 1), (5, 1)]
        Completed rewritten at 20210125, with reference to PUT-RP

        :ivar Profile profile: A Profile object that represents an election profile.
        :ivar List tie_breaking_order: the priority order of candidates, standing for the
              tiebreaking order, e.g. tie_breaking_order=[2,3,1,5,4], meaning 2>3>1>5>4
        """
        ...
        # print(wmg)
        I = list(wmg.keys())
        if tie_breaking_order is None:
            # tie_breaking_order = list(itertools.permutations(I, 2))  # modified 20210124
            tie_breaking_order = sorted(I)  # modified 20210127
        # tie_breaking_edges = sorted(list(itertools.permutations(tie_breaking_order, 2)), key=lambda x: -x[1])    # modified 20210127
        tie_breaking_edges = sorted(list(itertools.permutations(tie_breaking_order, 2)))  # modified 20210127
        G = nx.DiGraph()
        G.add_nodes_from(I)
        ...
        # self.output_graph(E, edge_color='r', width=1.5, label_pos=0.18, edge_labels_font_size=8)
        ...
        #        len(g.edges()) != 0]  # deprecated in Nexworkx>=2.1
        ...
        while len(E.edges) != 0:
            ...
            if ties == 1:
                ...
                tier = sorted(tier, key=tie_breaking_edges.index)  # modified 20210127
                while len(tier) != 0:
                    edge = tier.pop(0)  # modified 20210127
                    if nx.has_path(G, edge[1], edge[0]) is False:
                        G.add_edge(edge[0], edge[1])

        if len(E.edges) == 0:
            self.add_winners(G, I, known_winners, stats)
        # G_in_degree = G.in_degree(I)
        # winners = sorted([x[0] for x in G_in_degree if x[1] == 0])
        # self.output_graph(G, edge_color='b', width=1.5, label_pos=0.18, edge_labels_font_size=8)
        return sorted(known_winners)[0]#, G # single winner, modified 20210126

    def simple_ranked_pairs_old(self, profile, tie_breaking_order=None):
        """
        Returns a winner of profile under ranked pairs rule。
        N.B.: tie-breaking: the default tie-breaking is lexical order, e.g.
        [(1, 5), (2, 5), (3, 5), (4, 5), (1, 4), (2, 4), (3, 4), (5, 4), (1, 3),
        ...
        while len(E.edges) != 0:
            ...
            if ties > 1:
                # This tier has multiple edges with same max weight.
                tier = [(u, v) for (u, v, d) in E.edges(data=True) if d['weight'] == max_weight]
                max_edge = tie_breaking_order[min(tie_breaking_order.index(e) for e in tier)]

            E.remove_edges_from([max_edge])
            if nx.has_path(G, max_edge[1], max_edge[0]) is False:
                G.add_edge(max_edge[0], max_edge[1], weight=wmg[max_edge[0]][max_edge[1]])

        G_in_degree = G.in_degree(I)
        winners = sorted([x[0] for x in G_in_degree if x[1] == 0])
        # self.output_graph(G, edge_color='b', width=1.5, label_pos=0.18, edge_labels_font_size=8)
        return winners[0]  # , G # single winner, modified 20210124

    def output_graph(self, G, edge_color='r', width=3.0, label_pos=0.5, edge_labels_font_size=10):
        ...
        # edges
        nx.draw_networkx_edges(G, pos, width=width, alpha=0.5, edge_color=edge_color)
        ...
        # print(labels)
        nx.draw_networkx_edge_labels(G, pos, edge_labels=labels, label_pos=label_pos, alpha=0.5, font_size=edge_labels_font_size)
    ...
    def add_winners(self, G, I, known_winners, stats, possible_winners = None):
        ...
        # print("add winners,", known_winners)
    ...
    def stop_conditions(self, G, E, I, known_winners, stats):
        ...
        # print("pw", possible_winners, G.edges)
        ...
        if nx.is_directed_acyclic_graph(temp_G) is True:
            ...
            # self.add_winners(G, I, known_winners, stats)  # by Tyler before 20210124
            self.add_winners(temp_G, I, known_winners, stats)  # modified 20210124
            return 1

        return -1

    def get_tiers(self, E):
        Ec = E.copy()
    ...
    def getWinners(self, profile):
        ...
        tiers = self.get_tiers(E)
        # previous_orderings = set()
        # Sample
        num_samples = 10*len(wmg)
        for k in range(num_samples):
            self.sample2(I, tiers, known_winners, stats)

        stats.num_sampled = len(known_winners)
        # print("known_winners=", known_winners)
        ...
        while stackNode:
            ...
            while len(E.edges) != 0:
                ...
                if ties == 1:
                    ...
                    # print("G=", G.edges, "E=", E.edges, "tier=", tier)
    ...
    def explore_max_children_lp(self, G, tier, I, known_winners, stats, f_scc = 0):
        ...
        # print("chldren function: G=", G.edges, "T=", tier)
        cstack.append(root)
        ...
        while cstack:
            ...
            # Remove "redundant edges": if there is already path from e[0] to e[1], can immediately add e
            redundant_edges = set()
            # print("okkkkkkkkkkkk: G=", G.edges, "T=", T)
            for e in T:
                if G_tc.has_edge(e[0], e[1]):
                    redundant_edges.add(e)
                    G.add_edges_from([e])
    ...
    def sample(self, E, I, known_winners, stats):
        ...
        while len(Ec.edges()) != 0:
            ...
            for e in tier:
                ...
                # print(priority)
                # priorities.append(exp(priority / self.tau_for_testing))  # will overflow if large, before 20210124
                priorities.append(priority / self.tau_for_testing)  # modified 20210124
            #  ---------- modified 20210124 ----------
            max_priorities = max(priorities)
            priorities = [p - max_priorities for p in priorities]
            priorities = np.exp(priorities)
            q_sum = priorities.sum()
            #  ---------- ----------------- ----------
    ...
    def sample2(self, I, tied_edges, known_winners, stats):
        '''
        Using random tie-breaking, run through one procedure of RP and add resulting winner to known_winners
        :param E: DiGraph, All postive edges in the wmg
        :param I: List of all nodes in E
        ...
        # Create the order to add edges in each tier randomly
        tie_breaking_order = copy.deepcopy(tied_edges)

        for t in tie_breaking_order:
            random.shuffle(t)

        # if str(tie_breaking_order) in previous_orderings:
            # If it happened to randomly get the same ordering as already sampled, ignore this
            # Very rare due to the high number of permutations of tiers
            # return

        # previous_orderings.add(str(tie_breaking_order))

        for tier in tie_breaking_order:
            while len(tier) != 0:
                e = tier[0]
...
class MechanismRankedPairs_Old(MechanismEO):
    """
    The Ranked Pairs mechanism.
    This is the latest version created by Tyler Shepherd on
    Nov. 3 2018, originally in two_loop_LP_sampling_LP_11_3.py.
    ...
    def lexWinner_old(self, profile, tie_breaking_order=None):
        """
        Returns a winner of profile under ranked pairs rule。
        N.B.: tie-breaking: the default tie-breaking is lexical order, e.g.
        [(1, 5), (2, 5), (3, 5), (4, 5), (1, 4), (2, 4), (3, 4), (5, 4), (1, 3),
    ...
    def output_graph(self, G, edge_color='r', width=3, label_pos=0.5, edge_labels_font_size=10):
        # Draws the given graph G using networkx
        # print("Drawing!")
...
class MechanismBlack(Mechanism):
    """
    The Black mechanism.
    """
    def satCondorcet(self, profile):
...
class MechanismPluralityRunOff(Mechanism):
    """
...
class MechanismBordaMean(Mechanism):
    """
...
class MechanismRankedPairs_AAAI_fixed(MechanismEO):
    """
    The Ranked Pairs mechanism.
    """
    ...
    def getWinners(self, profile, prediction=[]):
        """
        Returns 1. a list of all PUT-winners of profile under ranked pairs rule
        and 2. A Stats object of runtime statistics
        ...
        num_samples = len(I)
        for k in range(num_samples):
            self.sample(E, I, tiers, previous_orderings, known_winners, stats)
    ...
    def sample(self, E, I, tied_edges, previous_orderings, known_winners, stats):
        '''
        Using random tie-breaking, run through one procedure of RP and add resulting winner to known_winners
        :param E: DiGraph, All postive edges in the wmg
        :param I: List of all nodes in E
        ...
        for tier in tie_breaking_order:
            while len(tier) != 0:
                ...
                G_in_degree = G.in_degree(I)
                no_incoming = set([x[0] for x in G_in_degree if x[1] == 0])
...
class MechanismRankedPairs_AAAI_Submission(MechanismEO):
    """
    The Ranked Pairs mechanism.
    """
